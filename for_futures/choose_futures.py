...
def callback_up(date, order_book_id, sym):
    if os.path.exists("futures.csv"):
        ...
        cur_dt = cur_dt.append([{'select_date': date, 'ts_code': order_book_id, 'symbol': sym}], ignore_index=True)
...
select(
   lambda: select_over_average(31) and select_long_average_up(5) and select_down_from_max(31, 1.07) and HHV(H, 21) / C > 1.07,
   start_date="20231210",
...
select(
   lambda: select_below_average(31) and select_long_average_down(3) and select_down_from_max(17, 1.05),
   start_date="20231210",
   end_date=trading_dates[-1],
