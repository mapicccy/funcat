...
class AkshareUSDataBackend(DataBackend):
    ...
    def trading_dates(self):
        ...
        df = self.ak.stock_us_daily(symbol='QQQ', adjust='qfq')
        # trade_date is an instance of datetime
        df = df.loc[df["date"] <= pd.to_datetime(now)]
    ...
    def get_price(self, order_book_id, start, end, freq):
        ...
        if 'df' not in dir():
            ...
            # akshare us daily not support pct_chg
    ...
    def get_order_book_id_list(self):
        ...
        filepath = os.path.join(cur_dir, "order_book_id_list_us.csv")
        try:
            if os.path.isfile(filepath):
                ...
                raise("No perm to access akshare stock us spot or order_book_id_list_us.csv doesn't exist!")
